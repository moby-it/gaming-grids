CREATE OR REPLACE FUNCTION public.get_most_popular(p_id uuid)
 RETURNS TABLE(common_cells text[], rarity_scores double precision[], champion_names text[])
 LANGUAGE plpgsql
AS $function$
DECLARE
    result TEXT[][] := ARRAY[['', '', ''], ['', '', ''], ['', '', '']];
    ...
    FOR i IN 1..3 LOOP
        FOR j IN 1..3 LOOP
            ...
            IF array_length(cell_values, 1) > 0 THEN
                ...
                SELECT c.name INTO champion_name
                FROM champion c
                WHERE c.champion_id = most_common
                LIMIT 1;
                champion_names[i][j] := champion_name;        
            ELSE
                result[i][j] := '';
        ...
        END LOOP;
    END LOOP;
    RETURN QUERY SELECT result, rarity_result, champion_names;
END;
$function$
;
